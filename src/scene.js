...
const Scene = (() => {
    ...
    // Wheel zoom that homes on the cursor: the camera moves toward the point
    // under the cursor so that point stays put on screen. It does NOT decide the
    // rotation pivot — that is chosen fresh when you start to rotate (see
    // recenterPivot), matching CAD navigation where zoom-to-cursor and the
    // orbit centre are independent.
    function onWheelZoom(e) {
        ...
        let scale = e.deltaY < 0 ? 0.85 : (1 / 0.85);   // <1 zoom in, >1 zoom out
        ...
        // Uniform scale of the camera+target rig about the cursor focus keeps
        // that focus point fixed on screen (true zoom-to-cursor).

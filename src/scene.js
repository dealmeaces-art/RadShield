...
const Scene = (() => {
    ...
    // Home View: snap the camera back to a standard, always-visible pose that
    // frames the whole scene — a front-elevated 3/4 view looking at the scene
    // center. Use this to recover if navigation leaves you lost in space.
    let lastBounds = null;
    function frameCameraTo(b) {
        if (!camera || !controls || !b) return;
        const cx = (b.minX + b.maxX) / 2, cy = (b.minY + b.maxY) / 2, cz = (b.minZ + b.maxZ) / 2;
        const maxDim = Math.max(b.maxX - b.minX, b.maxY - b.minY, b.maxZ - b.minZ, 20);
        camera.position.set(cx + maxDim * 0.55, cy + maxDim * 0.95, cz + maxDim * 1.7);
        controls.target.set(cx, cy, cz);
        controls.update();
    }
    function resetView() {
        frameCameraTo((lastBounds && isFinite(lastBounds.minX))
            ? lastBounds
            : { minX: -40, maxX: 40, minY: 0, maxY: 80, minZ: -40, maxZ: 40 });
    }
    ...
    function renderScene(volumesVisData, bounds, opts) {
        opts = opts || {};
        if (bounds && isFinite(bounds.minX)) lastBounds = bounds;   // for Home View
        ...
        // Frame camera to scene bounds (same canonical pose as Home View)
        if (bounds && opts.frame !== false) {
            frameCameraTo(bounds);
        }
    }
    ...
    function initEditorControls(callbacks) {
        ...
        el.addEventListener('pointerdown', (e) => {
            ...
            if (e.button === 0) {
                ...
                // A left-drag orbits — pivot about the centre of the current view
                if (!downOnGizmo) recenterPivot();
            } else if (e.button === 2) {
                rightX = e.clientX; rightY = e.clientY;
                recenterPivot();   // right-drag pans; keep the pivot sensible
            }
        });
    ...
    // Wheel zoom that homes on the cursor and NEVER stalls. Each notch dollies
    // the camera (and target, by the same vector, so orientation is untouched)
    // along the cursor ray. The step is proportional to the distance to what's
    // under the cursor — big steps far out, fine steps up close — but floored
    // at MIN_ZOOM_STEP: if you reach the focus point you simply push through it
    // and keep flying, you never hit a wall. Rotation pivot is chosen fresh on
    // drag start (recenterPivot), so zoom and orbit stay independent.
    function onWheelZoom(e) {
        ...
        // Distance to the geometry under the cursor (volume, isodose surface,
        // ground); falls back to the current pivot depth in empty space.
        let focusDist = null;
        ...
        if (hits.length) focusDist = hits[0].distance;
        if (focusDist == null) {
            const g = pickGroundPoint(rc);
            if (g) focusDist = camera.position.distanceTo(new THREE.Vector3(g.x, g.y, g.z));
        }
        if (focusDist == null) focusDist = camera.position.distanceTo(controls.target);

        const RATE = 0.15;           // fraction of the focus distance per notch
        const MIN_ZOOM_STEP = 3;     // cm — the push-through floor, never stalls
        const step = e.deltaY < 0
            ?  Math.max(focusDist * RATE, MIN_ZOOM_STEP)               // in
            : -Math.max(focusDist * RATE / (1 - RATE), MIN_ZOOM_STEP); // out (exact inverse)

        // Translate camera AND target by the same vector: the point under the
        // cursor stays pinned on screen and the view direction never changes.
        camera.position.addScaledVector(dir, step);
        controls.target.addScaledVector(dir, step);
        controls.update();
    }

    // Set the orbit pivot to whatever sits at the CENTER of the current view,
    // called when a rotate/pan drag begins. Because the view centre lies on the
    // camera's forward axis, moving the target along that axis changes only the
    // pivot depth — the on-screen image does not jump. This makes rotation spin
    // about the model you are looking at (Solid Edge rotates about the centre of
    // the model range), never about wherever you last zoomed.
    function recenterPivot() {
        if (!controls || !camera) return;
        const rc = new THREE.Raycaster();
        rc.setFromCamera(new THREE.Vector2(0, 0), camera);   // NDC (0,0) = screen centre
        let dist = null;
        const targets = pickMeshes.slice();
        if (isodoseMeshes && isodoseMeshes.length) targets.push(...isodoseMeshes);
        const hits = targets.length ? rc.intersectObjects(targets, false) : [];
        ...
        if (dist == null) {
            const g = pickGroundPoint(rc);
            if (g) dist = camera.position.distanceTo(new THREE.Vector3(g.x, g.y, g.z));
        }
        if (dist == null) return;   // nothing in the centre of view — keep the pivot
        dist = Math.max(controls.minDistance, Math.min(controls.maxDistance, dist));
        controls.target.copy(rc.ray.at(dist, new THREE.Vector3()));
        controls.update();
    }
    ...
    return {
        ...
        getCameraState: () => ({
            ...
            quat: camera ? camera.quaternion.toArray() : null
        }),
        COLORS
    };

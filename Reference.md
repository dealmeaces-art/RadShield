...
## What's New (v0.10 - July 2026) — tissue dose conversion + label layout
Portable file: `RadShield_Portable_v10.html`. Accuracy release driven by a side-by-side MicroShield benchmark in the field (1 Ci Co-60 ball, 2" steel + 2" lead, dose point 15" beyond — RadShield read 21.8 vs MicroShield 26 mR/hr).

- **Dose conversion: air kerma → absorbed dose to soft tissue (ICRU-44)** (`src/physics.js`, `src/materials.js`): the fluence→dose kernel previously used `(μ_en/ρ)_air` — that is air kerma, which reads ~10% below tissue dose and ~14% below an exposure (mR/hr) instrument, yet was labelled mrem/hr. It now uses NIST's ICRU-44 soft-tissue `μ_en/ρ` (new `Materials.getTissueMuEnRho`; table grid matches `ENERGY_GRID` exactly, direct transcription; deliberately NOT in `MATERIAL_DATA` so tissue is not a selectable shield and has no GP buildup set). **The whole 20% MicroShield gap was this convention**: on the benchmark, RadShield now computes 25.38 mrem/hr(tissue) vs MicroShield 26 mR/hr ≡ 25.3 mrem/hr(tissue) — agreement within 0.3%. Rule of thumb documented everywhere: `mR/hr ≈ mrem/hr ÷ 0.974`. Every reported number rises ~10% vs v0.9 (all suites are relative comparisons, so they pass unchanged; the buildup "exposure set" naming refers to the GP response function and is unrelated). Remaining benchmark residuals: RadShield honestly attenuates through a source ball's own material (a 1" **steel** ball self-absorbs ~38% at Co-60 energies — watch the source material when mimicking a "point source"), plus the ball-radius distance. The gamma-constant "unshielded reference" stays exposure-based (R/hr, sanity only, ~2.6% definitional offset).
- **Volume name labels: world-space layout with overlap stacking + leader lines** (`src/scene.js`): labels were placed at a canned local-frame offset `(r+12, h/2, 0)` — concentric tank layers (steel>lead>steel) all landed on the same spot and smooshed; rotated volumes carried labels to odd places (user-reported). Now `layoutVolumeLabels()` runs after all volumes build: each label anchors to the +X side of its volume's WORLD `Box3` at mid-height, crowding labels climb to the next free row (outermost layer keeps the base row), and a leader line ties each label to its own layer. Sprites stay parented to the volume group so they track live gizmo drags; `commitGizmoTransform` re-runs the layout on release. `Scene.getLabelLayout()` debug getter added for tests (scratchpad verify_labels.js: distinct rows, exact +150 cm follow, re-anchor after 90° rotation — passes on index.html and portable). Also fixed: label canvas textures now disposed on re-render (leak).
- Version stamps refreshed everywhere (guide cover/footer, design doc, printed report — the report had said v0.5 since v0.5).

## What's New (v0.9 - July 2026) — navigation rework + isodose performance & multi-source cleanup
Portable file: `RadShield_Portable_v9.html` (superseded by v10). Refinement wave driven by field use on large multi-source scenes.

- **Zoom-to-cursor camera + view-centered rotation** (`src/scene.js`): the built-in OrbitControls dolly is replaced by `onWheelZoom` — a **pure dolly that never stalls**. Each notch translates camera AND target by the same vector along the cursor ray (orientation untouched, cursor point stays pinned); the step is `focusDist × RATE` (RATE 0.15, focusDist = raycast to volumes → isodose → ground, else pivot depth) **floored at MIN_ZOOM_STEP (3 cm)** — reach the focus point and you push through it and keep flying, forever. (Two earlier versions were wrong: v1 welded the rotate pivot to the zoom point → whip-around; v2 scaled the rig about the focus but clamped against camera→target distance → asymptotic "brick wall" where zoom stopped dead even 100 ft from the model. Both user-reported.) Rotation pivot is decoupled: `recenterPivot()` fires on left/right pointer-down and sets `controls.target` to the geometry at the screen centre (NDC 0,0 raycast) — on the camera's forward axis, so only pivot *depth* changes, no view jump; rotation spins about what you're looking at (Solid Edge model: zoom to cursor, rotate about model/view centre). `controls.enableZoom=false`, `screenSpacePanning=true`. `Scene.getCameraState()` exposes pos/target/quat for tests. **Home View** (`Scene.resetView()`, `#homeViewBtn` in the ribbon "View" group) snaps the camera to a canonical front-elevated framed pose (shared `frameCameraTo(bounds)` helper, also used by renderScene's auto-frame and the sidebar Re-frame button) using the last rendered bounds — the recover-when-lost control. ⚠ **Open item:** wheel zoom sensitivity — RATE is 0.15/notch (~3× the old OrbitControls default ~5%); dial toward 0.08–0.10 when the user wants to tune the feel (user OK for now, 2026-07-09).
- **Isodose generation: responsive + faster** (`Physics.generateIsodoseSurfacesAsync`): the old generator ran as one synchronous main-thread block — long scenes (2 sources × 3 levels × 2500" range ≈ 35M ray traces) froze the page for minutes with "Page not responding" popups and the progress % never painted. New async generator (a) yields to the event loop every ~40 ms so the page stays live and progress paints (verified headlessly: 624 rAF frames + 65 progress states during a 70 s run vs ~0 before), and (b) does ONE outward march per ray that brackets ALL levels at once (the ×1.5 sample ladder is level-independent; per-level re-marching was pure duplicated work) — measured 1.75× fewer ray traces, 1.6× wall-clock (177 s → 110 s on the 2-source 2500" benchmark). Bisection still per level. **Results are bit-identical to the sync version** (regression test compares vertices to < 1e-9 cm). Sync `generateIsodoseSurfaces` kept for compat/tests. Used by both Analyze ▸ Generate Isodose Surfaces (`generateIsodose()` is now async) and Simulate ▸ Build field animation. NOTE: a distance-adaptive source-mesh coarsening (5–20× more) was proposed and REJECTED by the user — shielding geometry varies per element path even at long range (hose runs, shield-edge penumbras), so source meshing stays at full 5×16×8.
- **Isodose: early-exit + ray accel + Mid resolution** (second speed pass, user-approved after rejecting source-mesh coarsening): (a) **Early-exit-above** — the algorithm only ever asks "is dose above/below a level?", and element kernels are nonnegative, so `doseAtPoint(pos, breakAbove)` stops summing once the running total passes the relevant ceiling (march/s0/maxDist samples use the HIGHEST level as ceiling since they're compared against every level; bisection uses its own level). Same summation order ⇒ every decision, hence every vertex, is bit-identical. (b) **Bounding-sphere ray accel** — `SceneModel.buildRayAccel()` (one conservative world sphere per enabled volume) + optional `rayTrace(from, to, accel)` param: volumes whose sphere the segment misses are skipped in BOTH the intersect loop and the per-segment material lookup (`getMaterialAt` was rerunning `containsPoint` on every volume per segment). Pure rejection — layers identical (400-random-segment regression in geometry_check.js). Only the async generator passes accel (built fresh per generation; sync stays the untouched reference). (c) **Geodesic icosphere** — `createIcosphere(freq)` rewritten as a frequency-f geodesic (10f²+2 vertices), enabling **Mid = 362 rays** between Low 162 and Medium 642; `#isodoseRes` option values are now frequencies **4/6/8/16** (was subdivisions 2/3/4), Mid is the new default; generators accept `options.frequency` (legacy `subdivisions` still maps as 2^n). Measured on a 3-source, 1-level, 2500" scene: **1.66× wall-clock, output worstDelta=0**; combined with the Mid default, the user's ~25 min Medium runs drop to ~8–9 min at Mid. Regression: isodose_check section 5 pins the fully-optimized async against the unoptimized sync reference — still bit-identical.
- **Multi-source isodose dedupe** (`Scene.dedupeIsodoseSurfaces`, applied inside `renderIsodoseSurfaces` — display-only, physics data untouched): with several sources, merged lobes had both fans tracing the SAME total-dose boundary → two coincident translucent shells z-fighting, doubled wireframes and labels (user-reported). Fix: Voronoi triangle ownership — within each level, a fan keeps only triangles whose centroid is nearest its own source vantage; points not referenced by kept triangles are masked so labels stop doubling. Separated lobes are provably unchanged (regression: 80→80 faces); merged boundaries split cleanly at the midplane (80→44 per fan, 0 ownership violations). Input surface data never mutated (field-animation frames re-render safely). Minor cosmetic seam where the two tessellations meet at the midplane.

## What's New (v0.8 - July 2026) — mates, air gaps, heatmap rings, measure editing
Portable file: `RadShield_Portable_v8.html` (superseded by v9). A batch of workflow upgrades from field use on large scenes. Also introduced the first zoom-to-cursor camera — reworked in v0.9 (see above).
...
├── RadShield_Portable_v5..v9   # v5-v9 - outdated (kept locally; only current ships to GitHub)
├── RadShield_Portable_v10.html # v10 - CURRENT portable version (built, do not edit)
...
- **At work:** Email `RadShield_Portable_v10.html` to yourself, save to desktop, double-click to open in browser

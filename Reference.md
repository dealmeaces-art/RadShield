...
- **Zoom-to-cursor camera + view-centered rotation** (`src/scene.js`): the built-in OrbitControls dolly is replaced by `onWheelZoom`, which scales the camera+target rig about the point under the cursor (raycast volumes → isodose → ground) so that point stays fixed on screen — true zoom-to-cursor. The rotation pivot is **decoupled** from zoom (an early version wrongly welded the pivot to the zoom point → disorienting whip-around; fixed per Solid Edge's model: zoom to cursor, rotate about the center of the model/view). `recenterPivot()` fires on left/right pointer-down and sets `controls.target` to the geometry at the screen center (NDC 0,0 raycast); because that lies on the camera's forward axis, only the pivot *depth* changes — no view jump — and rotation spins about what you're looking at. `controls.enableZoom=false`, `screenSpacePanning=true`, min/max distance clamps. `Scene.getCameraState()` exposes pos/target/quat for tests. **Home View** (`Scene.resetView()`, `#homeViewBtn` in the ribbon "View" group) snaps the camera to a canonical front-elevated framed pose (shared `frameCameraTo(bounds)` helper, also used by renderScene's auto-frame and the sidebar Re-frame button) using the last rendered bounds — the recover-when-lost control. ⚠ **Open item:** wheel zoom feels more sensitive than the old OrbitControls default — `onWheelZoom` steps 0.85/notch (~15%) vs OrbitControls' ~5%; dial toward ~0.9–0.92 when revisited (user OK with it for now, 2026-07-09).
